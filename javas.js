...
var translations = {
	...
	pt_PT: {
		...
		Group: 'Grupo',
		'No possible snipe options found!': 'NÃ£o foram encontradas possÃ­veis opÃ§Ãµes de snipe!',
		Distance: 'DistÃ¢ncia',
		...
		'Configuration imported successfully!': 'ConfiguraÃ§Ã£o importada com sucesso!',
...
function resetGroup() {
	jQuery('#resetGroupBtn').on('click', function (e) {
		e.preventDefault();
		localStorage.removeItem('raSingleVillageSnipe_chosen_group');
...
function buildUnitsChoserTable() {
	...
	units.forEach((unit) => {
		if (unit !== 'spy' && unit !== 'militia') {
			...
			let checked = '';
			if (unit === 'spear' || unit === 'sword' || unit === 'archer' || unit === 'heavy' || unit === 'catapult') {
				checked = `checked`;
			}
...
async function fetchAllPlayerVillagesByGroup(groupId) {
	try {
		...
		if (game_data.player.sitter > 0) {
			...
			fetchVillagesUrl = game_data.link_base_pure + 'groups&ajax=load_villages_from_group';
		}
...
function countAPI() {
	...
	jQuery.getJSON(`https://api.countapi.xyz/hit/${author}/${prefix}`, function ({ value }) {
		console.debug(`${scriptInfo()} This script has been run ${formatAsNumber(parseInt(value))} times.`);
	});
}
